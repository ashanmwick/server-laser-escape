...
var import_Utils = require("@colyseus/core/utils/Utils");
...
function registerDevShutdownOnce() {
  ...
  (0, import_Utils.registerGracefulShutdown)((err) => currentServer?.gracefullyShutdown(true, err));
}
function getServerExport(mod) {
  return mod.server || mod.default?.server || import_core.Server.current;
}
...
function colyseus(options) {
  ...
  async function loadServerModule(httpServer) {
    ...
    try {
      ...
      const rooms = getRoomsExport(mod) || currentServer?.["~rooms"];
      const router = currentServer ? await (0, import_internal.prepareServices)(currentServer, !isStarted) : void 0;
      ...
      const envLatency = (0, import_core.parseLatencyEnv)();
      if (envLatency !== void 0) {
        (0, import_core.applySimulatedLatency)((0, import_Transport.getTransport)(), envLatency);
      }
      if (!options.quiet) {
