...
__export(Server_exports, {
  Server: () => Server,
  applySimulatedLatency: () => applySimulatedLatency,
  defineRoom: () => defineRoom,
  defineServer: () => defineServer,
  parseLatencyEnv: () => parseLatencyEnv,
...
function warnDevOnce(message) {
  ...
  import_Logger.logger.warn(message);
}
var DevServer = class extends Server {
  bootRuntime() {
    this.transport = (0, import_Transport.getTransport)();
    this._onTransportReady.resolve(this.transport);
    this.presence = matchMaker.presence;
    this.driver = matchMaker.driver;
...
function applySimulatedLatency(transport, milliseconds) {
  ...
  if (_originalRoomOnMessage == null) {
    _originalRoomOnMessage = import_Room.Room.prototype["_onMessage"];
  }
...
function parseLatencyEnv() {
  ...
  if (isNaN(milliseconds)) {
    import_Logger.logger.warn(`\u{1F4F6}\uFE0F\u2757 Ignoring COLYSEUS_LATENCY: expected a number of milliseconds, got ${JSON.stringify(value)}.`);
    return void 0;
  }
  return milliseconds;
}
...
function defineServer(options) {
  ...
  const server = import_DevMode.isDevMode ? new DevServer(serverOptions) : new Server(serverOptions);
