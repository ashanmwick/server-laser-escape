...
__export(InputBuffer_exports, {
  ...
  compileSanitizer: () => compileSanitizer,
  seedInputZeroValues: () => seedInputZeroValues,
  validateSubSteps: () => validateSubSteps
});
...
function fieldNamesOf(ctor) {
  const md = ctor[import_Utils.$METADATA];
  const names = [];
  if (md) {
    for (let i = 0; ; i++) {
      const f = md[i];
