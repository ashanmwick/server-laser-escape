...
function writeResponse(res, response, body) {
  res.cork(() => {
    res.writeStatus(`${response.status} ${response.statusText || import_http.STATUS_CODES[response.status] || ""}`);
...
var uWebSocketsTransport = class extends import_core.Transport {
  constructor(options = {}, appOptions = {}) {
    ...
    if (wsOptions.maxBackpressure === void 0) {
      wsOptions.maxBackpressure = 1024 * 1024;
    }
    if (wsOptions.compression === void 0) {
      wsOptions.compression = import_uWebSockets.default.DISABLED;
    }
    if (wsOptions.maxPayloadLength === void 0) {
      wsOptions.maxPayloadLength = 4 * 1024;
    }
    if (wsOptions.sendPingsAutomatically === void 0) {
    ...
    this.app.ws("/*", {
      ...
      upgrade: (res, req, context) => {
        ...
        const userData = {
          url,
          searchParams,
          context: (0, import_core.createAuthContext)({
            headers: rawHeaders,
            token: searchParams._authToken,
        ...
        (0, import_core.runBeforeUpgrade)(this._beforeUpgrade, query ? `${url}?${query}` : url, userData.context).then(async (response) => {
          if (response !== void 0) {
            const body = await response.arrayBuffer();
            if (!aborted) {
              writeResponse(res, response, body);
            }
            return;
          }
          if (!aborted) {
        ...
        }).catch(import_core.debugAndPrintError);
      },
      open: async (ws) => {
    ...
    if (!this._expressApp) {
      return new Promise(async (resolve, reject) => {
        try {
          ...
          this._expressApp = module2.default(this.app, { readBodyMaxTime: this._readBodyMaxTime });
  ...
  async onConnection(rawClient) {
    ...
    if (!sessionId && !roomId) {
      ...
      wrapper.on("message", (_) => {
        try {
          rawClient.send(new Uint8Array([import_core.Protocol.PING]), true);
        } catch (e) {
        }
      });
