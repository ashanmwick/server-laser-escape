// packages/testing/src/Room.ext.ts
import { Deferred, Room } from "@colyseus/core";
import { Room as ClientRoom } from "@colyseus/sdk";
var _originalOnMessage = Room.prototype["_onMessage"];
Room.prototype["_onMessage"] = function() {
  _originalOnMessage.apply(this, arguments);
  if (this._waitingForMessage) {
    setTimeout(() => this._waitingForMessage[1].resolve(), this._waitingForMessage[0]);
  }
};
Room.prototype.waitForNextMessage = async function(additionalDelay = 0) {
  this._waitingForMessage = [additionalDelay, new Deferred()];
  return this._waitingForMessage[1];
};
Room.prototype.waitForMessage = async function(type, rejectTimeout = 3e3) {
  const originalHandlers = this["onMessageEvents"].events[type] || [];
  const room = this;
  return new Promise((resolve, reject) => {
    const rejectionTimeout = setTimeout(() => reject(new Error(`message '${type}' was not called. timed out (${rejectTimeout}ms)`)), rejectTimeout);
    room["onMessageEvents"].events[type] = [
      async function(client, message) {
        clearTimeout(rejectionTimeout);
        for (const handler of originalHandlers) {
          await handler.call(room, client, message);
        }
        room["onMessageEvents"].events[type] = originalHandlers;
        resolve([client, message]);
      }
    ];
  });
};
Room.prototype.waitForNextTimestep = async function() {
  if (!this["_simulationInterval"]) {
    console.warn("\u26A0\uFE0F waitForNextTimestep() - the room must call .setTimestep() or .setFixedTimestep().");
    return;
  }
  return new Promise((resolve) => (this._waitingForTimestep ??= []).push(resolve));
};
function releaseTimestepWaiters(room) {
  const waiting = room._waitingForTimestep;
  if (waiting === void 0 || waiting.length === 0) {
    return;
  }
  room._waitingForTimestep = [];
  for (const resolve of waiting) {
    resolve();
  }
}
var _originalSetTimestep = Room.prototype.setTimestep;
Room.prototype.setTimestep = function(onTickCallback, delay) {
  if (onTickCallback === void 0) {
    return _originalSetTimestep.call(this, onTickCallback, delay);
  }
  return _originalSetTimestep.call(this, (deltaTime) => {
    try {
      return onTickCallback(deltaTime);
    } finally {
      releaseTimestepWaiters(this);
    }
  }, delay);
};
var _originalSetFixedTimestep = Room.prototype.setFixedTimestep;
Room.prototype.setFixedTimestep = function(step, tickRate, opts) {
  return _originalSetFixedTimestep.call(this, (ctx) => {
    try {
      return step(ctx);
    } finally {
      releaseTimestepWaiters(this);
    }
  }, tickRate, opts);
};
Room.prototype.waitForNextSimulationTick = function() {
  return this.waitForNextTimestep();
};
var _originalBroadcastPatch = Room.prototype["broadcastPatch"];
Room.prototype["broadcastPatch"] = function() {
  const retVal = _originalBroadcastPatch.call(this);
  if (this._waitingForPatch) {
    setTimeout(() => this._waitingForPatch[1].resolve(), this._waitingForPatch[0]);
  }
  return retVal;
};
Room.prototype.waitForNextPatch = async function(additionalDelay = 0) {
  this._waitingForPatch = [additionalDelay, new Deferred()];
  return this._waitingForPatch[1];
};
ClientRoom.prototype.waitForMessage = async function(type, rejectTimeout = 3e3) {
  return new Promise((resolve, reject) => {
    const received = (message) => {
      unbind();
      resolve(message);
      clearTimeout(rejectionTimeout);
    };
    const unbind = this["onMessageHandlers"].on(type, (message) => received(message));
    const rejectionTimeout = setTimeout(() => {
      unbind();
      reject(new Error(`message '${type}' was not called. timed out (${rejectTimeout}ms)`));
    }, rejectTimeout);
  });
};
var _originalClientOnMessage = ClientRoom.prototype["dispatchMessage"];
ClientRoom.prototype["dispatchMessage"] = function() {
  _originalClientOnMessage.apply(this, arguments);
  if (this._waitingForMessage) {
    setTimeout(() => {
      this._waitingForMessage[1].resolve([arguments[0], arguments[1]]);
    }, this._waitingForMessage[0]);
  }
};
ClientRoom.prototype.waitForNextMessage = async function(additionalDelay = 0) {
  this._waitingForMessage = [additionalDelay, new Deferred()];
  return this._waitingForMessage[1];
};
ClientRoom.prototype.waitForNextPatch = async function(additionalDelay = 0) {
  return new Promise((resolve) => this.onStateChange.once(() => setTimeout(resolve, additionalDelay)));
};
ClientRoom.prototype.waitForInitialState = async function() {
  if (this.serializerId === "none") {
    return;
  }
  return new Promise((resolve) => {
    this.onStateChange.once(() => resolve());
    this.onLeave.once(() => resolve());
  });
};
