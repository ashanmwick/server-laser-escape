...
class StateTreeView {
    constructor(container, getState) {
        ...
        this.container.addEventListener('click', (e) => {
            const button = e.target.closest('[data-expand-button]');
            if (button) {
                e.stopPropagation();
     ...
     */
    collectPatches(changes) {
        const patches = [];
        for (let i = 0; i < changes.length; i++) {
            const change = changes[i];
            if (!isPatchablePrimitive(change.value)) {
                return null;
            }
            const ownerId = this.refIds.get(change.ref);
    ...
    buildValueIndex() {
        this.valueIndex = new Map();
        const spans = this.container.querySelectorAll('[data-vk]');
    ...
    renderNode(obj, depth = 0, parentKey = '', path = '', fieldName = null, vk = null) {
        if (depth > 10) {
    ...
    renderKeyValue(key, value, depth, currentPath, keyDisplay, isKeyString, ownerId) {
 ...
 */
function schemaFieldKeys(obj) {
    if (schema.Schema.isSchema(obj)) {
        return Object.keys(schema.Metadata.getFields(obj.constructor));
    }
...
function isPatchablePrimitive(value) {
    if (value === null) {
        return true;
    }
...
function renderExpandButton(displayState, displayLabel, size) {
...
function formatPrimitive(value) {
    if (value === null) {
        return { text: 'null', color: VALUE_COLORS.nullish };
    }
    if (value === undefined) {
        return { text: 'undefined', color: VALUE_COLORS.nullish };
    }
    switch (typeof value) {
 ...
 */
function primitiveSpan(value, vk) {
...
exports.StateTreeView = StateTreeView;
//# sourceMappingURL=state-tree.cjs.map
