...
function createMenu(logoContainer) {
    ...
    function makeSectionHeader(iconSvg, text) {
        ...
        h.innerHTML = icons.sizedIcon(iconSvg, 12) + '<span>' + text + '</span>';
        return h;
    }
    ...
    function updateHostDisplay() {
        var hostText = 'N/A';
        var connected = core.roomDebugInfo.size > 0;
        if (connected) {
            // Get host from first room
            var firstRoom = core.roomDebugInfo.values().next().value;
    ...
    latencyValueSpan.textContent = core.preferences.latencySimulation.delay + 'ms';
    ...
    latencySlider.min = '0';
    latencySlider.max = core.preferences.maxLatency.toString();
    latencySlider.value = core.preferences.latencySimulation.delay.toString();
    ...
    function getSliderColor(value) {
        var c = core.latencyRamp(value, core.preferences.maxLatency);
    ...
    function trackGradient(value) {
        ...
        var valuePercent = (value / core.preferences.maxLatency) * 100;
        var yellowColor = getSliderColor(core.preferences.maxLatency / 2);
        return (value <= core.preferences.maxLatency / 2)
    ...
    function refreshLogoRing() {
        var container = core.getDebugRoot().getElementById('debug-logo-container');
        if (container) {
    ...
    latencySlider.addEventListener('input', function () {
        ...
        core.preferences.latencySimulation.delay = value;
        core.preferences.latencySimulation.enabled = value > 0 || core.preferences.latencySimulation.jitter > 0;
    ...
    jitterValueSpan.textContent = core.preferences.latencySimulation.jitter + 'ms';
    ...
    jitterSlider.min = '0';
    jitterSlider.max = core.preferences.maxLatency.toString();
    jitterSlider.value = core.preferences.latencySimulation.jitter.toString();
    ...
    jitterSlider.addEventListener('input', function () {
        ...
        core.preferences.latencySimulation.jitter = value;
        core.preferences.latencySimulation.enabled = core.preferences.latencySimulation.delay > 0 || value > 0;
    ...
    function activePresetIndex() {
        var d = core.preferences.latencySimulation.delay;
        var j = core.preferences.latencySimulation.jitter;
        for (var i = 0; i < NET_PRESETS.length; i++) {
            if (NET_PRESETS[i].delay === d && NET_PRESETS[i].jitter === j)
                return i;
        }
        return -1; // custom
    }
    ...
    function syncPresetButtons() {
        ...
        for (var i = 0; i < presetButtons.length; i++) {
            tokens.applySegmentedState(presetButtons[i], i === active);
        }
    }
    ...
    var presetLabel = makeSectionHeader(icons.networkIcon, 'Network simulation');
    ...
    NET_PRESETS.forEach(function (preset) {
        ...
        tokens.applySegmentedState(btn, false);
        tokens.bindSegmentedHover(btn);
    ...
    var authLabel = makeSectionHeader(icons.keyIcon, 'Auth token');
    ...
    var copyBtnIcon = icons.sizedIcon(icons.copyIcon, 13);
    ...
    function getAuthToken() {
        var t = null;
        core.authInstances.forEach(function (a) { if (!t && a.token) {
            t = a.token;
        } });
        if (t) {
            return t;
        ...
        try {
            return localStorage.getItem('colyseus-auth-token');
        }
        catch (e) {
            return null;
        }
    ...
    function copyToken() {
        ...
        try {
            ...
            if (p && p.then) {
                p.then(flashCopied, flashCopied);
            }
            else {
                flashCopied();
            }
    ...
    clearBtn.addEventListener('click', function (e) {
        ...
        core.authInstances.forEach(function (a) { if (a.token) {
            void a.signOut();
        } });
        try {
            localStorage.removeItem('colyseus-auth-token');
        }
        catch (e) { } // default-key fallback
        syncAuthOption();
    });
    ...
    positionColumn.appendChild(makeSectionHeader(icons.settingsIcon, 'Position'));
    ...
    function syncPositionButtons() {
        for (var i = 0; i < positionButtons.length; i++) {
            tokens.applySegmentedState(positionButtons[i], positionButtons[i].dataset.value === core.preferences.panelPosition.position);
        }
    }
    POSITIONS.forEach(function (pos) {
        ...
        dot.style.cssText = 'position:absolute;width:10px;height:6px;border-radius:2px;' +
            'background:currentColor;' + geometry.verticalEdge(pos.value) + ':6px;' + geometry.horizontalEdge(pos.value) + ':6px';
        btn.appendChild(dot);
        tokens.applySegmentedState(btn, false);
        tokens.bindSegmentedHover(btn);
        btn.addEventListener('click', function (e) {
            e.stopPropagation();
            core.preferences.panelPosition.position = pos.value;
            core.savePreferences();
            layout.reflow(); // logo, menu, drawer and Predict all anchor off this
            syncPositionButtons();
        });
    ...
    visibilityColumn.appendChild(makeSectionHeader(icons.eyeSlashIcon, 'Visibility'));
    ...
    hideBtn.innerHTML = icons.sizedIcon(icons.eyeSlashIcon, 14) + '<span>Hide for this session</span>';
    ...
    core.getDebugRoot().appendChild(menu);
    ...
    function setMenuVisible(visible) {
        ...
        core.setCompactMenuOpen(visible);
        if (visible) {
            updateHostDisplay();
    ...
    logoContainer.addEventListener('pointerdown', function (e) {
        if (!layout.isCompact() || pressTimer) {
            return;
        }
    ...
    logoContainer.addEventListener('pointermove', function (e) {
        if (!pressTimer) {
            return;
        }
    ...
    logoContainer.addEventListener('click', function (e) {
        ...
        if (longPressFired) {
            longPressFired = false;
            return;
        } // the press already opened the menu
        if (menuVisible) {
            setMenuVisible(false);
            return;
        } // tap dismisses the sheet first
        core.setCompactRevealed(!core.isCompactRevealed());
    });
