 ...
 */
function toDebugHandle(core) {
    const fieldsByProfile = new Map();
    core.onTrack((profileIdx, field) => {
        let s = fieldsByProfile.get(profileIdx);
        if (!s) {
            s = new Set();
            fieldsByProfile.set(profileIdx, s);
        }
        s.add(field);
    });
    return {
        ...
        profiles: () => core.profiles().map((p) => (Object.assign(Object.assign({}, p), { fields: fieldsByProfile.get(p.id) ? [...fieldsByProfile.get(p.id)] : [] }))),
    };
}

exports.toDebugHandle = toDebugHandle;
//# sourceMappingURL=predict-bridge.cjs.map
