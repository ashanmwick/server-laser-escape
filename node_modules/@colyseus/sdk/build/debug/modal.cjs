...
document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape' && modalStack.length > 0) {
        ...
        if (topModal && core.getDebugRoot().contains(topModal)) {
            topModal.remove();
        }
    }
});
 ...
 */
function applyModalLayout(modal, opts) {
    var handles = modal.querySelectorAll('.resize-handle');
    if (layout.isCompact()) {
        // Flush to the safe area on all four edges — setInset carries the px fallback
        // for engines that can't parse max()/env().
        ['top', 'right', 'bottom', 'left'].forEach(function (side) { geometry.setInset(modal, side, 0); });
    ...
    if (opts.maxHeight) {
        geometry.setMaxViewportHeight(modal, opts.maxHeight);
    }
    else {
...
function createModal(options) {
    ...
    var stopReflow = layout.onReflow(function (state) {
...
function createModalHeader(options) {
    ...
    header.style.cursor = (opts.draggable !== false && !layout.isCompact()) ? 'move' : 'default';
    ...
    closeButton.innerHTML = icons.closeIcon;
...
function makeDraggable(modal, dragHandle) {
    ...
    var onPointerDown = function (e) {
        ...
        if (layout.isCompact()) {
            return;
        } // full-screen sheet has nowhere to go
...
exports.applyModalLayout = applyModalLayout;
