...
import { getDebugRoot } from './core.mjs';
import { setInset, setMaxViewportHeight } from './geometry.mjs';
import { isCompact, onReflow } from './layout.mjs';
...
function applyModalLayout(modal, opts) {
    ...
    if (isCompact()) {
        ...
        ['top', 'right', 'bottom', 'left'].forEach(function (side) { setInset(modal, side, 0); });
    ...
    if (opts.maxHeight) {
        setMaxViewportHeight(modal, opts.maxHeight);
    }
    else {
...
export { applyModalLayout, createModal, createModalHeader, makeDraggable };
