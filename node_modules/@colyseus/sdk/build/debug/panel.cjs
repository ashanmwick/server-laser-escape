...
var sendMessage = require('./send-message.cjs');
var stateInspector = require('./state-inspector.cjs');

function initialize() {
    if (core.isPanelsHidden())
        return;
    ...
    container.style.border = '3px solid ' + core.getBorderColor(core.preferences.latencySimulation.delay, 0.7);
    ...
    core.applyPanelPosition();
    // container on hover effect
    container.addEventListener('mouseenter', function () {
        container.style.backgroundColor = 'rgba(0, 0, 0, 0.9)';
        container.style.borderColor = core.getBorderColor(core.preferences.latencySimulation.delay, 0.9);
    });
    container.addEventListener('mouseleave', function () {
        container.style.backgroundColor = 'rgba(0, 0, 0, 0.7)';
        container.style.borderColor = core.getBorderColor(core.preferences.latencySimulation.delay, 0.7);
    });
    ...
    icon.insertAdjacentHTML('beforeend', icons.logoIcon);
    container.appendChild(icon);
    core.getDebugRoot().appendChild(container);
    // Create menu first
...
function createDebugPanel(uniquePanelId, debugInfo) {
    ...
    var existingPanel = core.getDebugRoot().getElementById('debug-panel-' + uniquePanelId);
    if (existingPanel) {
    ...
    panel.style.display = core.isPanelsHidden() ? 'none' : 'block';
    ...
    title.innerHTML = '<span id="debug-title-text-' + uniquePanelId + '"><span class="debug-room-name"></span><span class="debug-info-icon" style="display: inline-flex; align-items: center; margin-left: 4px; cursor: pointer; opacity: 0.6; vertical-align: middle;">' + icons.resizeIcon(icons.infoIcon, 10) + '</span></span><span style="display:flex;align-items:center;gap:8px;font-weight:normal"><span id="debug-ping-' + uniquePanelId + '" style="font-size: 10px; color: #888;" title="Ping time">--</span><span class="debug-chevron" style="color:#888;display:none">▾</span></span>';
    ...
    function createActionButton(id, icon, label, onClick) {
        ...
        btn.innerHTML = icons.sizedIcon(icon, 12) + '<span>' + label + '</span>';
    ...
    var stateBtn = createActionButton('debug-state-btn-' + uniquePanelId, icons.treeViewIcon, 'State', function () { stateInspector.openStateInspectorModal(uniquePanelId); });
    var messageBtn = createActionButton('debug-message-btn-' + uniquePanelId, icons.messageIcon, 'Send', function () { sendMessage.openSendMessagesModal(uniquePanelId); });
    ...
    var disconnectBtn = createActionButton('debug-disconnect-btn-' + uniquePanelId, icons.disconnectIcon, 'Drop', function () {
        var info = core.roomDebugInfo.get(uniquePanelId);
    ...
    title.addEventListener('click', function () {
        if (!layout.isCompact()) {
            return;
        } // desktop: always expanded, header inert
    ...
    var stack = core.getPanelStack();
    stack.insertBefore(panel, stack.querySelector('#' + core.PREDICT_CONTAINER_ID));
    return panel;
}
// Chip ⇄ full panel. The chevron only makes sense where the header is tappable.
function setPanelExpanded(panel, expanded) {
    var body = panel.querySelector('[id^="debug-body-"]');
    var title = panel.querySelector('.debug-title');
    var chevron = panel.querySelector('.debug-chevron');
...
function bindTooltipDismiss() {
    document.addEventListener('click', function () {
        var tooltips = core.getDebugRoot().querySelectorAll('[id^="debug-tooltip-"]');
...
function watchPanelShape() {
    ...
    layout.onReflow(function (state) {
...
function updateDebugPanel(uniquePanelId, debugInfo) {
    ...
    html += '<div style="margin-bottom: 4px;"><div style="display: flex; align-items: center; gap: 6px;"><span style="display: inline-flex; align-items: center; width: 18px; height: 18px; color: #FF9800;">' + icons.envelopeUp + '</span><span style="color: #FF9800;">' + core.formatBytes(debugInfo.bytesSentPerSec) + '/s</span></div><div style="margin-left: 24px; opacity: 0.7; font-size: 9px;">' + debugInfo.messagesSentPerSec.toFixed(0) + ' messages</div></div>';
    html += '<div' + (hasJitter ? ' style="margin-bottom: 4px;"' : '') + '><div style="display: flex; align-items: center; gap: 6px;"><span style="display: inline-flex; align-items: center; width: 18px; height: 18px; color: #2196F3;">' + icons.envelopeDown + '</span><span style="color: #2196F3;">' + core.formatBytes(debugInfo.bytesReceivedPerSec) + '/s</span></div><div style="margin-left: 24px; opacity: 0.7; font-size: 9px;">' + debugInfo.messagesReceivedPerSec.toFixed(0) + ' messages</div></div>';
    // Jitter: third stat row, same shape as sent/received (icon + label, value below).
    if (hasJitter) {
        html += '<div><div style="display: flex; align-items: center; gap: 6px;"><span style="display: inline-flex; align-items: center; width: 18px; height: 18px; color: ' + jitterAccent + ';">' + icons.jitterIcon + '</span><span style="color: ' + jitterAccent + ';">jitter</span></div><div style="margin-left: 24px; opacity: 0.7; font-size: 9px;">±' + jitterText + '</div></div>';
    }
    html += '</div>';
    ...
    setTimeout(function () {
        ...
        if (hasJitter)
            drawGraph('graph-jitter-' + uniquePanelId, debugInfo.jitterHistory, jitterAccent);
    }, 10);
}
// Draw graph on canvas
function drawGraph(canvasId, data, color) {
    var canvas = core.getDebugRoot().getElementById(canvasId);
    if (!canvas)
        return;
...
exports.updateDebugPanel = updateDebugPanel;
