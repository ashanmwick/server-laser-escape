    ...
    try {
        ...
        if (prefs.jitter !== undefined && prefs.jitter !== null) {
            ...
            if (!isNaN(jitterValue) && jitterValue >= 0) {
                ...
                if (jitterValue > 0)
                    preferences.latencySimulation.enabled = true;
            }
        }
...
function getDebugRoot() {
    ...
    styles.injectBaseStyles(_debugShadowRoot);
...
function anchorEdges() {
    ...
    return { vertical: geometry.verticalEdge(position), horizontal: geometry.horizontalEdge(position) };
}
/**
 ...
 */
function latencyRamp(value, max) {
...
function applyPanelPosition() {
    if (panelsHidden)
        return; // nothing mounted — don't force the shadow root into existence
    ...
    if (logoContainer) {
        geometry.clearEdges(logoContainer);
        geometry.setInset(logoContainer, vertical, geometry.EDGE_INSET);
        geometry.setInset(logoContainer, horizontal, geometry.EDGE_INSET);
    }
    if (menu) {
        applyMenuPosition(menu, vertical, horizontal);
    }
 ...
 */
function applyCompactBox(el, vertical, horizontal) {
    geometry.setInset(el, 'left', geometry.EDGE_INSET);
    geometry.setInset(el, 'right', geometry.EDGE_INSET);
    geometry.setInset(el, vertical, geometry.EDGE_INSET, COMPACT_LOGO_CLEARANCE);
    ...
    var reserved = geometry.EDGE_INSET + COMPACT_LOGO_CLEARANCE + geometry.EDGE_INSET;
    geometry.setMaxViewportHeight(el, `calc(100vh - ${reserved}px)`);
}
function clearCompactBox(el) {
    el.style.maxWidth = '';
    el.style.marginLeft = '';
...
function applyMenuPosition(menu, vertical, horizontal) {
    geometry.clearEdges(menu);
    ...
    if (layout.isCompact()) {
        ...
        geometry.setInset(menu, horizontal, geometry.EDGE_INSET);
        ...
        geometry.setMaxViewportHeight(menu, `calc(100vh - ${MENU_OFFSET + geometry.EDGE_INSET}px)`);
    }
}
const PREDICT_CONTAINER_ID = 'colyseus-debug-predict-container';
...
function isCompactRevealed() { return compactRevealed; }
/** Show/hide the compact overlay content. The logo itself always stays put. */
function setCompactRevealed(revealed) {
...
function applyOverlayVisibility() {
    ...
    if (predictContainer) {
        predictContainer.style.display = display;
    }
}
...
function repositionDebugPanels() {
    if (panelsHidden)
        return;
    var stack = getDebugRoot().getElementById(PANEL_STACK_ID);
    if (!stack)
        return;
    var edges = anchorEdges();
    var vertical = edges.vertical;
    var horizontal = edges.horizontal;
    geometry.clearEdges(stack);
    if (layout.isCompact()) {
        ...
        geometry.setInset(stack, vertical, geometry.EDGE_INSET);
        geometry.setInset(stack, horizontal, geometry.EDGE_INSET, LOGO_CLEARANCE);
    }
}
function isPanelsHidden() { return panelsHidden; }

exports.PREDICT_CONTAINER_ID = PREDICT_CONTAINER_ID;
exports.applyOverlayVisibility = applyOverlayVisibility;
exports.applyPanelPosition = applyPanelPosition;
exports.authInstances = authInstances;
exports.formatBytes = formatBytes;
exports.getBorderColor = getBorderColor;
exports.getDebugRoot = getDebugRoot;
exports.getPanelStack = getPanelStack;
exports.hidePanelsForSession = hidePanelsForSession;
exports.isCompactRevealed = isCompactRevealed;
exports.isOverlayVisible = isOverlayVisible;
exports.isPanelsHidden = isPanelsHidden;
exports.latencyRamp = latencyRamp;
exports.loadPreferences = loadPreferences;
exports.preferences = preferences;
exports.repositionDebugPanels = repositionDebugPanels;
exports.roomDebugInfo = roomDebugInfo;
exports.savePreferences = savePreferences;
exports.setCompactMenuOpen = setCompactMenuOpen;
exports.setCompactRevealed = setCompactRevealed;
