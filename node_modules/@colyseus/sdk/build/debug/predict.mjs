...
//
// colyseus.js@0.18.2
import { PREDICT_CONTAINER_ID, isOverlayVisible, getPanelStack, getDebugRoot, preferences } from './core.mjs';
import { clearEdges, verticalEdge, horizontalEdge, setInset, setMaxViewportHeight } from './geometry.mjs';
import { applySegmentedState, bindSegmentedHover, RULE, SECONDARY, SEGMENT } from './tokens.mjs';
import { isCompact, isCoarsePointer } from './layout.mjs';
import { drawGraph } from './panel.mjs';
import { toDebugHandle } from './predict-bridge.mjs';
...
const ALL_MODES = ["lerp", "extrapolate", "damped", "reckon", "raw"];
// Compact glyphs so all five modes fit on one line; the mode name is the tooltip.
const ICON_ATTRS = 'width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" ' +
 ...
 * replays on install.
 */
function onPredictPublished(core) {
    if (panels.has(core.name))
        return; // dedupe
 ...
 */
function syncHeaderNames() {
    const show = panels.size > 1;
    for (const entry of panels.values()) {
        const nameEl = entry.el.querySelector('[data-role="name"]');
        if (nameEl)
            nameEl.textContent = show ? ` ${entry.handle.name}` : "";
    }
}
// -----------------------------------------------------------------------------
// Container
 ...
 */
function positionPredictContainer() {
    if (!container)
        return;
...
function readSessionSet(storageKey) {
    try {
...
function updateSessionSet(storageKey, member, present) {
...
const setCollapsed = (name, collapsed) => updateSessionSet(COLLAPSE_KEY, name, collapsed);
// -----------------------------------------------------------------------------
...
function createPill(value, active, onClick) {
...
function renderModePills(host, current, onSelect) {
    host.innerHTML = "";
    for (const m of ALL_MODES) {
        host.appendChild(createPill(m, m === current, () => {
            for (const sib of host.querySelectorAll("button")) {
...
function syncModePills(host, current) {
    for (const pill of host.querySelectorAll("button")) {
...
function renderCard(handle) {
    ...
    card.querySelector('[data-role="attached"]').textContent = String(handle.attachedCount());
 ...
 */
function renderProfileSubcards(card, handle) {
    const host = card.querySelector('[data-role="profiles"]');
    ...
    if (host.dataset.key !== key) {
        ...
        for (let i = 0; i < profiles.length; i++) {
            refreshProfileSubcard(nodes[i], handle, profiles[i]);
        }
    }
}
function buildProfileSubcard(handle, p) {
    ...
    `;
    const modesEl = el.querySelector('[data-role="modes"]');
    ...
    const tune = el.querySelector('[data-role="tune"]');
 ...
 */
function syncTuning(subcard) {
    const sliders = subcard.querySelector('[data-role="sliders"]');
    const tune = subcard.querySelector('[data-role="tune"]');
    const chevron = subcard.querySelector('[data-role="tune-chevron"]');
    const label = subcard.querySelector('[data-role="tune-label"]');
...
function refreshProfileSubcard(el, handle, p) {
    ...
    const slidersEl = el.querySelector('[data-role="sliders"]');
    if (slidersEl.dataset.mode !== p.mode) {
        renderProfileSliders(el, handle, p.id, p.mode);
    }
}
function renderProfileSliders(subcard, handle, id, mode) {
    const slidersEl = subcard.querySelector('[data-role="sliders"]');
    ...
    if (mode === "lerp") {
        addSlider(slidersEl, "delay (ms)", 0, 300, p.delay, (v) => handle.setProfile(id, { delay: v }));
        addSlider(slidersEl, "tickInterval (ms, 0=off)", 0, 100, p.tickInterval ?? 0, (v) => handle.setProfile(id, { tickInterval: v }));
        addSlider(slidersEl, "smoothMs (0=off)", 0, 300, p.smoothMs, (v) => handle.setProfile(id, { smoothMs: v }));
    }
    else if (mode === "extrapolate") {
        addSlider(slidersEl, "maxExtrapolate (ms)", 0, 300, p.maxExtrapolate, (v) => handle.setProfile(id, { maxExtrapolate: v }));
        addSlider(slidersEl, "tickInterval (ms, 0=off)", 0, 100, p.tickInterval ?? 0, (v) => handle.setProfile(id, { tickInterval: v }));
        addSlider(slidersEl, "smoothMs (0=off)", 0, 300, p.smoothMs, (v) => handle.setProfile(id, { smoothMs: v }));
    }
    else if (mode === "damped") {
        addSlider(slidersEl, "smoothMs (0=snap)", 0, 300, p.smoothMs, (v) => handle.setProfile(id, { smoothMs: v }));
    }
    if (mode === "lerp" || mode === "extrapolate" || mode === "damped") {
        // Value units are game-specific — widen the range to fit the config.
        const snap = p.snap ?? 0;
        addSlider(slidersEl, "snap (units, 0=off)", 0, Math.max(20, Math.ceil(snap * 2)), snap, (v) => handle.setProfile(id, { snap: v }));
    }
...
// -----------------------------------------------------------------------------
function addSlider(parent, label, min, max, initial, onChange) {
    ...
    const num = row.querySelector('[data-role="val"]');
...
// Live polling
// -----------------------------------------------------------------------------
function updateCardLive(handle, entry) {
    entry.el.querySelector('[data-role="attached"]').textContent =
...
function fmtDrift(v) {
    if (v < 1e-3) {
        return "0";
    } // below any status floor — float noise
    if (v < 1) {
        return v.toFixed(3);
    } // keep small REAL drift legible (0.003)
    if (v < 10) {
        return v.toFixed(2);
    }
    return v.toFixed(1);
...
function graphId(name, i) {
    return `predict-rc-graph-${name.replace(/[^a-z0-9]/gi, "-")}-${i}`;
}
function renderReconcilers(entry, handle) {
    const host = entry.el.querySelector('[data-role="reconcilers"]');
    ...
    if (host.childElementCount !== stats.length) {
        ...
        for (let i = 0; i < stats.length; i++) {
            host.appendChild(buildReconcilerRow(graphId(handle.name, i)));
        }
    }
    for (let i = 0; i < stats.length; i++) {
        refreshReconcilerRow(host.children[i], stats[i], entry, graphId(handle.name, i));
    }
}
function buildReconcilerRow(canvasId) {
...
function refreshReconcilerRow(row, stat, entry, canvasId) {
    let hist = entry.histories.get(stat.label);
    if (!hist) {
        hist = [];
        entry.histories.set(stat.label, hist);
    }
    ...
    row.querySelector('[data-role="rc-label"]').textContent = stat.label;
    const statusEl = row.querySelector('[data-role="rc-status"]');
...
export { onPredictPublished, positionPredictContainer };
