...
var stateTree = require('./state-tree.cjs');

// Create and open State Inspector modal
function openStateInspectorModal(uniquePanelId) {
    var _a;
    var debugInfo = core.roomDebugInfo.get(uniquePanelId);
    ...
    var existingModal = core.getDebugRoot().getElementById('debug-state-inspector-modal');
    if (existingModal) {
    ...
    function saveStateInspectorPreferences() {
        try {
            var rect = modal$1.getBoundingClientRect();
            var prefs = {
                width: rect.width,
                height: rect.height,
                left: rect.left,
    ...
    const modal$1 = modal.createModal(modalOpts);
    // Create header using shared utility
    const headerComponents = modal.createModalHeader({
    ...
    var stateView = new stateTree.StateTreeView(contentContainer, function () { return room.state || {}; });
    stateView.render();
    ...
    modal$1.appendChild(contentContainer);
    core.getDebugRoot().appendChild(modal$1);
    ...
    header.addEventListener('pointerdown', function (e) {
        if (layout.isCompact()) {
            return;
        } // full-screen sheet: nothing to drag
    ...
    var handleMouseMove = function (e) {
        if (isResizing && resizeHandle) {
            ...
            // Apply constraints
            newWidth = Math.max(parseInt(modal$1.style.minWidth) || 300, Math.min(newWidth, window.innerWidth - newLeft));
            newHeight = Math.max(parseInt(modal$1.style.minHeight) || 200, Math.min(newHeight, window.innerHeight - newTop));
            modal$1.style.width = newWidth + 'px';
            modal$1.style.height = newHeight + 'px';
            modal$1.style.left = newLeft + 'px';
            modal$1.style.top = newTop + 'px';
            modal$1.style.transform = 'none';
        }
        else if (isDragging) {
            // Handle drag
            ...
            var maxX = window.innerWidth - modal$1.offsetWidth;
            var maxY = window.innerHeight - modal$1.offsetHeight;
            newX = Math.max(0, Math.min(newX, maxX));
            newY = Math.max(0, Math.min(newY, maxY));
            modal$1.style.left = newX + 'px';
            modal$1.style.top = newY + 'px';
            modal$1.style.transform = 'none';
        }
    };
    ...
    var resizeHandleSize = layout.isCoarsePointer() ? 12 : 8;
    var cornerHandleSize = layout.isCoarsePointer() ? 20 : 12; // Larger handles for corners to make them easier to grab
    ...
    modal.applyModalLayout(modal$1, modalOpts);
    // Remove state change listener when modal is closed
    var originalRemove = modal$1.remove;
...
exports.openStateInspectorModal = openStateInspectorModal;
//# sourceMappingURL=state-inspector.cjs.map
