...
//
// colyseus.js@0.18.2
import { newDrift, updateDrift, classifyDrift, resetDrift } from './drift.mjs';
import { warnReadBeforePump, diagnosticsActive, warnDivergence, warnMemoCollision } from './divergence.mjs';
...
class MemoStore {
     ...
     */
    run(tick, isReplay, key, compute) {
        if (isReplay)
            return this.byTick.get(tick)?.get(key);
        if (diagnosticsActive()) {
            if (tick !== this.diagTick) {
                this.diagTick = tick;
                this.diagKeys.clear();
            }
...
function isReplaying() { return replayDepth > 0; }
/**
...
class RollbackController {
    ...
    ackWatermark = () => this.input_.lastProcessed;
    ...
    constructor(opts) {
        ...
        if (stepMs === undefined) {
            throw new Error("@colyseus/sdk reconciler: fixed simulation step is unknown. The " +
        ...
        this.stepCtx = {
            ...
            predict: (sink, payload) => { if (!this.stepCtx.isReplay)
                sink._predictFromSim(this.stepCtx.tick, payload, this.ackWatermark); },
        };
     ...
     */
    noteRenderRead() {
        if (this.catching || this.stepMs <= 0 || this.renderAcc < this.stepMs)
            return;
    ...
    catchUp() {
        ...
        if (this.predictedSeq >= sent || this.catching)
            return;
        ...
        for (let seq = this.predictedSeq + 1; seq <= sent; seq++) {
            ...
            if (inp !== undefined) {
                ...
                if (this.renderAcc < 0)
                    this.renderAcc = 0;
                else if (this.renderAcc >= this.stepMs)
                    this.renderAcc %= this.stepMs;
            }
    ...
    reconcile(acked) {
        ...
        for (const f of this.smoothedFields())
            renderedBefore[f] = this.readCurrent(f) + this.error[f];
        this.adoptTruth();
    ...
    onDisposed(hook) { this.disposedHooks.push(hook); }
    ...
    dispose() {
        ...
        for (const hook of this.disposedHooks.splice(0))
            hook();
    }
    /**
     ...
     */
    truthMatchesAt(_acked) { return false; }
...
export { RollbackController, isReplaying };
