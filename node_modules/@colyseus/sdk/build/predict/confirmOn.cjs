 ...
 */
function wireConfirmOn(callbacks, channel, on, sessionId) {
    const offs = [];
    if (on.field !== undefined) {
        ...
        offs.push(callbacks.onAdd(on.collection, (child, key) => {
            if (listeners.has(child))
                return; // decoder can re-fire onAdd for one ref
            listeners.set(child, callbacks.listen(child, on.field, (v) => { if (v === on.equals)
                channel.confirm(key); }, 
            // no immediate fire: a child already flipped at bind time is
            // history, not a settle signal (would fire onUnpredicted)
            false));
        }));
        offs.push(callbacks.onRemove(on.collection, (child) => {
            var _a;
            (_a = listeners.get(child)) === null || _a === void 0 ? void 0 : _a();
        ...
        offs.push(callbacks.onRemove(on.collection, (_child, key) => {
            channel.confirm(key);
        }));
    }
    else {
        ...
        offs.push(callbacks.onAdd(on.collection, (child) => {
            if (on.mine !== undefined && child[on.mine] !== (sessionId === null || sessionId === void 0 ? void 0 : sessionId()))
                return;
            channel.confirm();
        }, false));
    }
...
exports.wireConfirmOn = wireConfirmOn;
//# sourceMappingURL=confirmOn.cjs.map
