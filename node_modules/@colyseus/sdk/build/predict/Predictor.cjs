...
var predictedEventChannel = require('./predictedEventChannel.cjs');
var predictedSpawns = require('./predictedSpawns.cjs');
var reconciler = require('./reconciler.cjs');
var simReconciler = require('./simReconciler.cjs');
var InputHandle = require('../input/InputHandle.cjs');
var drift = require('./drift.cjs');
...
var confirmOn = require('./confirmOn.cjs');
...
const RECKON_DEFAULTS = {
    step: undefined,
    smoothMs: 50,
    substep: 16,
    snap: 0,
};
function isReckonAttachConfig(cfg) {
    if (cfg === null || typeof cfg !== "object")
        return false;
    return Array.isArray(cfg.fields);
}
...
function warnNoInputClock() {
    if (__warnedNoInputClock) {
        return;
    }
    __warnedNoInputClock = true;
    console.warn("@colyseus/sdk Predict: room.clock isn't server-synced because the server " +
...
class Predict {
    ...
    constructor(room, opts) {
        ...
        this.slotBuf = new Float64Array(64 * SLOT_STRIDE);
        this.slotCount = 0;
        this.slotDetach = [];
        ...
        this.profileBuf = new Float64Array(8 * PROFILE_STRIDE);
        this.profileCount = 0;
        this.profileKeys = new Map();
        /**
        ...
        this.computeLerp = (slotIdx) => {
            ...
            const dt = now - lastT;
            if (dt <= 0)
                return buf[i + SLOT_AUX_V]; // same-frame re-read
         ...
         */
        this.computeExtrapolate = (slotIdx) => {
            const buf = this.slotBuf;
            const i = slotIdx * SLOT_STRIDE;
            const pBuf = this.profileBuf;
            const pBase = (buf[i + SLOT_PROFILE] | 0) * PROFILE_STRIDE;
        ...
        this.computeReckon = (slotIdx) => {
            ...
            if (sim !== undefined) {
                ...
                if (pos >= 0)
                    return this.applySimulation(sim, pos);
            }
            return buf[i + SLOT_V1];
        };
        /**
        ...
        const isSmoothingDefault = this.defaultMode !== "reckon" && this.defaultMode !== "raw";
        const initial = isSmoothingDefault
            ? (() => {
                var _a, _b, _c, _d, _e, _f, _g;
                const s = rest;
                return {
                    mode: ((_a = s.mode) !== null && _a !== void 0 ? _a : SMOOTHING_DEFAULTS.mode),
                    delay: (_b = s.delay) !== null && _b !== void 0 ? _b : SMOOTHING_DEFAULTS.delay,
                    smoothMs: (_c = s.smoothMs) !== null && _c !== void 0 ? _c : SMOOTHING_DEFAULTS.smoothMs,
                    maxExtrapolate: (_d = s.maxExtrapolate) !== null && _d !== void 0 ? _d : SMOOTHING_DEFAULTS.maxExtrapolate,
                    tickInterval: (_e = s.tickInterval) !== null && _e !== void 0 ? _e : SMOOTHING_DEFAULTS.tickInterval,
                    snap: (_f = s.snap) !== null && _f !== void 0 ? _f : SMOOTHING_DEFAULTS.snap,
                    angle: (_g = s.angle) !== null && _g !== void 0 ? _g : SMOOTHING_DEFAULTS.angle,
                };
            })()
            : Object.assign({}, SMOOTHING_DEFAULTS);
        ...
        const lerpSmoothMs = isSmoothingDefault ? ((_b = rest.smoothMs) !== null && _b !== void 0 ? _b : 0) : 0;
        const dIdx = this.allocProfile(this.defaultMode, initial.delay, initial.smoothMs, lerpSmoothMs, initial.maxExtrapolate, initial.tickInterval, initial.snap, false);
        ...
        if (this.defaultMode === "reckon") {
            ...
            this.reckonDefaults = {
                step: r.step,
                smoothMs: (_c = r.smoothMs) !== null && _c !== void 0 ? _c : RECKON_DEFAULTS.smoothMs,
                substep: (_d = r.substep) !== null && _d !== void 0 ? _d : RECKON_DEFAULTS.substep,
                snap: (_e = r.snap) !== null && _e !== void 0 ? _e : RECKON_DEFAULTS.snap,
            };
        }
        else {
            this.reckonDefaults = Object.assign({}, RECKON_DEFAULTS);
        }
        ...
        // forwardMs/elapsedMs.
        this.clock = (_f = clock !== null && clock !== void 0 ? clock : room.clock) !== null && _f !== void 0 ? _f : undefined;
        ...
        this.useRenderClock = rest.renderPresent !== false;
        ...
        if (this.clock === RoomClock.NULL_CLOCK) {
            warnNoInputClock();
        }
        this.name = (_g = rest.name) !== null && _g !== void 0 ? _g : `predict#${++__predictAutoId}`;
        ...
        debugChannel.publishDebug("predict", this.makeCoreHandle());
    }
    /**
    ...
    makeCoreHandle() {
        return {
            ...
            reckonDefaults: () => (Object.assign({}, this.reckonDefaults)),
            ...
            onTrack: (cb) => {
                ...
                return () => {
                    ...
                    if (i >= 0)
                        this.trackListeners.splice(i, 1);
                };
            },
            onDispose: (cb) => {
                ...
                return () => {
                    ...
                    if (i >= 0)
                        this.disposeListeners.splice(i, 1);
                };
            },
        };
    }
    ...
    snapshotReconcilers() {
        ...
        for (const d of this.driven) {
            ...
            out.push({
                label: `reconciler #${i++}`,
                status: drift.classifyDrift(r.drift, tol),
                severity: tol !== undefined && tol > 0 ? r.drift.ema / tol : undefined,
                ema: r.drift.ema,
                peak: r.drift.peak,
                lastCorrectionMag: (_a = r.lastCorrectionMag) !== null && _a !== void 0 ? _a : 0,
                reconcileSeq: (_b = r.reconcileSeq) !== null && _b !== void 0 ? _b : 0,
            });
        }
        return out;
    }
    readSmoothingDefaults() {
        const p = this.profileBuf;
        const b = DEFAULTS_PROFILE * PROFILE_STRIDE;
        const m = p[b + P_MODE] | 0;
        const mode = m === MODE_LERP ? "lerp" :
     ...
     * profile, not 1000.
     */
    allocProfile(mode, delay, smoothMs, lerpSmoothMs, maxExtrapolate, tickInterval, snap, dedup, label) {
        ...
        if (dedup) {
            ...
            key = `${label !== null && label !== void 0 ? label : ""}|${modeCode}|${delay}|${smoothMs}|${lerpSmoothMs}|${maxExtrapolate}|${tickInterval}|${snap}`;
        ...
        if (dedup)
            this.profileKeys.set(key, idx);
        return idx;
    }
    computerForMode(modeCode) {
        if (modeCode === MODE_LERP)
            return this.computeLerp;
        if (modeCode === MODE_EXTRAPOLATE)
            return this.computeExtrapolate;
        if (modeCode === MODE_DAMPED)
            return this.computeDamped;
        if (modeCode === MODE_RECKON)
            return this.computeReckon;
        if (modeCode === MODE_BOUND)
            return this.computeBound;
    ...
    profileFromOpts(opts) {
        ...
        const mode = ((_a = opts.mode) !== null && _a !== void 0 ? _a : d.mode);
        const delay = (_b = opts.delay) !== null && _b !== void 0 ? _b : d.delay;
        const smoothMs = (_c = opts.smoothMs) !== null && _c !== void 0 ? _c : d.smoothMs;
        const lerpSmoothMs = (_d = opts.smoothMs) !== null && _d !== void 0 ? _d : d.lerpSmoothMs;
        const maxExtrapolate = (_e = opts.maxExtrapolate) !== null && _e !== void 0 ? _e : d.maxExtrapolate;
        const tickInterval = (_f = opts.tickInterval) !== null && _f !== void 0 ? _f : d.tickInterval;
        const snap = (_g = opts.snap) !== null && _g !== void 0 ? _g : d.snap;
        ...
        if (mode === d.mode && delay === d.delay && smoothMs === d.smoothMs &&
     ...
     * groups never do.
     */
    groupProfile(opts, label) {
        var _a, _b, _c, _d, _e, _f, _g;
        const d = this.readSmoothingDefaults();
        const mode = ((_a = opts.mode) !== null && _a !== void 0 ? _a : this.defaultMode);
        return this.allocProfile(mode, (_b = opts.delay) !== null && _b !== void 0 ? _b : d.delay, (_c = opts.smoothMs) !== null && _c !== void 0 ? _c : d.smoothMs, (_d = opts.smoothMs) !== null && _d !== void 0 ? _d : d.lerpSmoothMs, (_e = opts.maxExtrapolate) !== null && _e !== void 0 ? _e : d.maxExtrapolate, (_f = opts.tickInterval) !== null && _f !== void 0 ? _f : d.tickInterval, (_g = opts.snap) !== null && _g !== void 0 ? _g : d.snap, true, label);
    }
    /**
    ...
    setDefaults(opts) {
        ...
        if (s.delay !== undefined)
            p[base + P_DELAY] = s.delay;
        // Explicit smoothMs arms both fields — lerp's spring and damped/
        // extrapolate's rate — so the value survives runtime mode flips.
        if (s.smoothMs !== undefined) {
            p[base + P_SMOOTH_MS] = s.smoothMs;
            p[base + P_LERP_SMOOTH_MS] = s.smoothMs;
        }
        if (s.maxExtrapolate !== undefined)
            p[base + P_MAX_EXTRAPOLATE] = s.maxExtrapolate;
        if (s.tickInterval !== undefined)
            p[base + P_TICK_INTERVAL] = s.tickInterval;
        if (s.snap !== undefined) {
            p[base + P_SNAP] = s.snap;
            this.reckonDefaults.snap = s.snap;
        }
        // Reckon-mode fields → reckonDefaults.
        const r = opts;
        if (r.step !== undefined)
            this.reckonDefaults.step = r.step;
        if (r.smoothMs !== undefined)
            this.reckonDefaults.smoothMs = r.smoothMs;
        if (r.substep !== undefined)
            this.reckonDefaults.substep = r.substep;
    }
    /**
    ...
    snapshotProfiles() {
        ...
        for (let i = 0; i < this.profileCount; i++) {
            ...
            if (m === MODE_BOUND)
                continue; // controller-owned — nothing tunable
            const mode = m === MODE_LERP ? "lerp" :
                m === MODE_EXTRAPOLATE ? "extrapolate" :
                    m === MODE_DAMPED ? "damped" :
                        m === MODE_RECKON ? "reckon" : "raw";
     ...
     */
    setProfile(id, opts) {
        if (id < 0 || id >= this.profileCount)
            return;
        const base = id * PROFILE_STRIDE;
        const p = this.profileBuf;
        if ((p[base + P_MODE] | 0) === MODE_BOUND)
            return; // controller-owned — not tunable
        if (opts.mode !== undefined) {
            ...
            if (id === DEFAULTS_PROFILE)
                this.defaultMode = opts.mode;
        }
        if (opts.delay !== undefined)
            p[base + P_DELAY] = opts.delay;
        if (opts.smoothMs !== undefined) {
            p[base + P_SMOOTH_MS] = opts.smoothMs;
            p[base + P_LERP_SMOOTH_MS] = opts.smoothMs;
        }
        if (opts.maxExtrapolate !== undefined)
            p[base + P_MAX_EXTRAPOLATE] = opts.maxExtrapolate;
        if (opts.tickInterval !== undefined)
            p[base + P_TICK_INTERVAL] = opts.tickInterval;
        if (opts.snap !== undefined)
            p[base + P_SNAP] = opts.snap;
    }
    /**
    ...
    dispose() {
        var _a, _b;
        for (const refId of new Set([...this.slotByRef.keys(), ...this.simByRef.keys()]))
            this.detachByRef(refId);
        this.trackListeners.length = 0;
        for (const d of this.driven.splice(0))
            (_b = (_a = d).dispose) === null || _b === void 0 ? void 0 : _b.call(_a);
        for (const cb of this.disposeListeners.splice(0))
            cb();
    }
    ...
    track(instance, field, opts = {}) {
        ...
        return this.trackWithProfile(instance, field, this.profileFromOpts(opts), (_a = opts.angle) !== null && _a !== void 0 ? _a : false);
    }
    /**
     ...
     */
    trackWithProfile(instance, field, profileIdx, angle = false) {
        var _a, _b;
        ...
        const refId = schemaReflect.refIdOf(instance);
        if (refId === undefined) {
            throw new Error("Predict.track(): instance has no refId — track/attach must run " +
                "AFTER the decoder delivers the instance (e.g. inside onAdd).");
        }
        const fieldId = schemaReflect.fieldIndexOf(instance, field);
        ...
        const initial = (_a = instance[field]) !== null && _a !== void 0 ? _a : 0;
        ...
        let boundOver;
        const existingIdx = (_b = this.slotByRef.get(refId)) === null || _b === void 0 ? void 0 : _b.get(field);
        if (existingIdx !== undefined) {
            boundOver = this.boundBySlot.get(existingIdx);
            if (boundOver !== undefined)
                this.freeStash(boundOver);
            else
        ...
        if (boundOver !== undefined) {
            ...
            if (perRef === undefined) {
                perRef = new Map();
                this.slotByRef.set(refId, perRef);
            }
        ...
        const detach = this.callbacks.listen(instance, field, (current) => {
            var _a;
            ...
            if (angle) {
                const prev = b[i + SLOT_V1];
                current = prev + Math.atan2(Math.sin(current - prev), Math.cos(current - prev));
            }
            ...
            if (count >= 2 && isFinite(lastT1)) {
                ...
                const patchMs = ((_a = this.clock) === null || _a === void 0 ? void 0 : _a.patchInterval) ? this.clock.patchInterval() : 0;
     ...
     */
    untrack(instance, field) {
        const refId = schemaReflect.refIdOf(instance);
        if (refId === undefined)
            return;
        this.untrackSlot(refId, field);
    }
    untrackSlot(refId, field) {
        var _a, _b;
        const perRef = this.slotByRef.get(refId);
        const slotIdx = perRef === null || perRef === void 0 ? void 0 : perRef.get(field);
        ...
        (_b = (_a = this.slotDetach)[slotIdx]) === null || _b === void 0 ? void 0 : _b.call(_a);
        this.slotDetach[slotIdx] = undefined;
        this.freeSlots.push(slotIdx);
        perRef.delete(field);
        if (perRef.size === 0)
            this.slotByRef.delete(refId);
    }
    ...
    freeStash(entry) {
        ...
        if (entry.stash < 0)
            return;
        (_b = (_a = this.slotDetach)[entry.stash]) === null || _b === void 0 ? void 0 : _b.call(_a);
     ...
     */
    registerBound(ctrl, source, fields, poseKeys, profileIdx) {
        var _a;
        const refId = schemaReflect.refIdOf(source); // callers pre-check (bound sources are decoded)
        let perRef = this.slotByRef.get(refId);
        if (perRef === undefined) {
            perRef = new Map();
            this.slotByRef.set(refId, perRef);
        ...
        for (let k = 0; k < fields.length; k++) {
            ...
            if (existing !== undefined) {
                ...
                if (prior !== undefined) {
                    ...
                    console.warn(`@colyseus/sdk Predict: "${field}" (refId ${refId}) is already bound ` +
            ...
            // teardown paths use `?.()`).
            const initial = (_a = source[field]) !== null && _a !== void 0 ? _a : 0;
            buf[base + SLOT_V1] = initial;
            buf[base + SLOT_AUX_V] = initial;
            ...
            buf[base + SLOT_PROFILE] = profileIdx;
            buf[base + SLOT_REF] = refId;
            buf[base + SLOT_FIELD] = schemaReflect.fieldIndexOf(source, field);
            buf[base + SLOT_RING_HEAD] = 0;
            buf[base + SLOT_RING_COUNT] = 0;
        ...
        return () => {
            for (let k = 0; k < slots.length; k++) {
                ...
                if (entry === undefined || entry.ctrl !== ctrl)
                    continue;
                this.boundBySlot.delete(slotIdx);
                this.freeSlots.push(slotIdx);
                const pr = this.slotByRef.get(refId);
                if ((pr === null || pr === void 0 ? void 0 : pr.get(fields[k])) === slotIdx) {
                    if (entry.stash >= 0) {
    ...
    installBoundOverlay(ctrl) {
        const offs = [];
        for (const reg of ctrl.boundRegistrations) {
            if (reg.fields.length === 0 || schemaReflect.refIdOf(reg.source) === undefined)
                continue;
        ...
        if (offs.length > 0)
            ctrl.onDisposed(() => { for (const off of offs)
                off(); });
    }
    /**
     ...
     */
    trackStepped(instance, opts) {
        var _a, _b, _c, _d;
        const substep = (_a = opts.substep) !== null && _a !== void 0 ? _a : 16;
        const fields = opts.fields;
        const fieldIds = fields.map((f) => schemaReflect.fieldIndexOf(instance, f));
        ...
        const forwardMs = (_b = opts.forwardMs) !== null && _b !== void 0 ? _b : (present
            ? () => { const stamp = clock.lastServerTime(); return stamp > 0 ? Math.max(0, present() - stamp) : 0; }
            : () => 0);
        const elapsedMs = (_c = opts.elapsedMs) !== null && _c !== void 0 ? _c : (present !== null && present !== void 0 ? present : (() => performance.now()));
        // Build `advance` once. It writes predicted fields into the SoA `out`
        // buffer (indexed by field position) — never a name-keyed object.
        const liveValues = instance[schemaReflect.$VALUES];
        ...
        const fastPathOk = opts.snapshot === undefined &&
            Array.isArray(liveValues) &&
            fieldIds.every((id) => id >= 0 && id < liveValues.length);
        let advance;
        if (fastPathOk) {
            ...
            const scratch = new instance.constructor();
            const sv = scratch[schemaReflect.$VALUES];
            advance = (live, fwd, out, endElapsed) => {
                const lv = live[schemaReflect.$VALUES];
            ...
            const fieldNames = schemaReflect.scalarFieldNamesOf(instance);
            const snapshotFn = (_d = opts.snapshot) !== null && _d !== void 0 ? _d : (fieldNames.length > 0
                ? schemaReflect.makeUnrolledSnapshot(fieldNames)
                : (e) => (Object.assign({}, e)));
            const names = [...fields];
            advance = (live, fwd, out, endElapsed) => {
                const scratch = snapshotFn(live);
                let remaining = fwd;
    ...
    trackSimulated(instance, opts) {
        var _a, _b, _c, _d;
        const refId = schemaReflect.refIdOf(instance);
        if (refId === undefined) {
            throw new Error("Predict.trackStepped(): instance has no refId — must run AFTER " +
        ...
        for (let k = 0; k < n; k++)
            smoothed[k] = (_a = instance[fields[k]]) !== null && _a !== void 0 ? _a : 0;
        ...
        const elapsedMs = (_b = opts.elapsedMs) !== null && _b !== void 0 ? _b : (present !== null && present !== void 0 ? present : (() => performance.now()));
        const state = {
            instance,
            fieldIds,
            posOf,
            ...
            smoothMs: (_c = opts.smoothMs) !== null && _c !== void 0 ? _c : 50,
            snap: (_d = opts.snap) !== null && _d !== void 0 ? _d : 0,
            smoothed,
     ...
     */
    resolveCtorPlan(group, child) {
        var _a;
        const md = schemaReflect.metadataOf(child);
        const typeName = ((_a = child.constructor) === null || _a === void 0 ? void 0 : _a.name) || "?";
        const config = group.config;
        if (!isReckonAttachConfig(config)) {
            // Per-field smoothing map.
            if (md === undefined)
                return group.basePlan; // non-schema fixture
            const keys = Object.keys(config).filter((k) => config[k] !== undefined);
            if (keys.every((k) => typeof md[k] === "number"))
                return group.basePlan;
            const filtered = {};
            for (const k of keys)
                if (typeof md[k] === "number")
                    filtered[k] = config[k];
            return this.buildGroupPlan(filtered, `${group.label}:${typeName}`);
        }
        const rcfg = config;
        let fields = rcfg.fields;
        if (md !== undefined) {
            const filtered = fields.filter((f) => typeof md[f] === "number");
            if (filtered.length !== fields.length)
                fields = filtered;
        }
        if (fields === rcfg.fields)
            return group.basePlan; // all fields present
        return this.buildGroupPlan(Object.assign(Object.assign({}, rcfg), { fields }), `${group.label}:${typeName}`);
    }
    /**
    ...
    buildGroupPlan(config, label) {
        ...
        if (isReckonAttachConfig(config)) {
            ...
            const effectiveMode = (_a = rcfg.mode) !== null && _a !== void 0 ? _a : this.defaultMode;
            const step = (_b = rcfg.step) !== null && _b !== void 0 ? _b : this.reckonDefaults.step;
            const isReckon = effectiveMode === "reckon";
            if (isReckon && typeof step !== "function") {
                throw new Error("Predict.attach(): reckon mode requires a 'step' function. " +
            ...
            for (const f of rcfg.fields)
                fieldProfiles.push({ field: f, profileIdx, angle: rcfg.angle });
            return {
                label,
                isReckon,
                reckonFields: isReckon ? rcfg.fields : undefined,
                reckonStep: isReckon ? step : undefined,
                reckonSmoothMs: (_c = rcfg.smoothMs) !== null && _c !== void 0 ? _c : this.reckonDefaults.smoothMs,
                reckonSubstep: (_d = rcfg.substep) !== null && _d !== void 0 ? _d : this.reckonDefaults.substep,
                reckonSnap: (_e = rcfg.snap) !== null && _e !== void 0 ? _e : this.reckonDefaults.snap,
        ...
        for (const key of Object.keys(smoothing)) {
            const value = smoothing[key];
            if (value === undefined)
                continue;
            const o = typeof value === "string" ? { mode: value } : value;
            fieldProfiles.push({ field: key, profileIdx: this.groupProfile(o, label), angle: o.angle });
        }
    ...
    attachToGroup(instance, plan, forwardMs) {
        const offs = [];
        if (plan.isReckon) {
            offs.push(this.trackStepped(instance, {
                fields: plan.reckonFields,
                step: plan.reckonStep,
                smoothMs: plan.reckonSmoothMs,
                substep: plan.reckonSubstep,
                snap: plan.reckonSnap,
                snapshot: plan.reckonSnapshot,
                forwardMs,
            }));
        }
        for (const { field, profileIdx, angle } of plan.fieldProfiles) {
            offs.push(this.trackWithProfile(instance, field, profileIdx, !!angle));
        }
        return () => { for (const f of offs)
            f(); };
    }
    attachWithPlan(instance, plan) {
        const refId = schemaReflect.refIdOf(instance);
        if (refId === undefined) {
            throw new Error("Predict.attach(): instance has no refId — attach must run AFTER " +
                "the decoder delivers the instance (e.g. inside onAdd).");
        }
    ...
    attachAll(...args) {
        ...
        const parent = rootForm ? undefined : args[0];
        const key = rootForm ? args[0] : args[1];
        const config = rootForm ? args[1] : args[2];
        ...
        return () => {
            addOff === null || addOff === void 0 ? void 0 : addOff();
            removeOff === null || removeOff === void 0 ? void 0 : removeOff();
    ...
    tick(now = performance.now()) {
        ...
        for (let i = 0; i < driven.length; i++) {
            ...
            if (d.dead)
                continue;
            (_a = d.tick) === null || _a === void 0 ? void 0 : _a.call(d, now);
            (_b = d.prune) === null || _b === void 0 ? void 0 : _b.call(d);
            if (live !== i)
                driven[live] = d;
            live++;
        }
        if (live !== driven.length)
            driven.length = live;
        return steps;
    }
    // --- Event channels --------------------------------------------------------
    ...
    defineEvent(opts) {
        var _a;
        const { confirmOn: confirmOn$1 } = opts, channelOpts = tslib.__rest(opts, ["confirmOn"]);
        const channel = new predictedEventChannel.PredictedEventChannel(channelOpts, (_a = this.clock) !== null && _a !== void 0 ? _a : null);
        if (confirmOn$1 !== undefined) {
            if (confirmOn$1.mine !== undefined && typeof this.sessionSource.sessionId !== "string") {
                throw new Error(`Predict.defineEvent: confirmOn.mine ("${confirmOn$1.mine}") requires a Predict ` +
                    `built from a Room (it compares against room.sessionId); confirm manually instead.`);
            }
            channel._addTeardown(confirmOn.wireConfirmOn(this.callbacks, channel, confirmOn$1, () => this.sessionSource.sessionId));
        }
    ...
    spawns(key, opts = {}) {
        var _a, _b;
        const store = new predictedSpawns.PredictedSpawns(opts, (_a = this.clock) !== null && _a !== void 0 ? _a : null);
        ...
        const group = fields !== undefined && step !== undefined
            ? this.makeGroup(String(key), {
                ...
                fields,
                step: step,
                smoothMs: (_b = opts.smoothMs) !== null && _b !== void 0 ? _b : 0,
                substep: opts.substep,
            })
            : undefined;
        const untrack = group !== undefined ? new Map() : undefined;
        const clock = this.clock;
        store.attach((onAdd, onRemove) => {
            const addOff = this.callbacks.onAdd(key, (server, k) => {
                var _a, _b;
                onAdd(server, k);
                if (group === undefined || untrack.has(server))
                    return; // decoder re-fire
                const lead = (_b = (_a = store.entryFor(server)) === null || _a === void 0 ? void 0 : _a.leadMs) !== null && _b !== void 0 ? _b : 0;
                const forwardMs = clock
                    ...
                    : () => Math.max(0, lead);
                untrack.set(server, this.attachToGroup(server, this.planFor(group, server), forwardMs));
            });
            const removeOff = this.callbacks.onRemove(key, (server, k) => {
                var _a;
                (_a = untrack === null || untrack === void 0 ? void 0 : untrack.get(server)) === null || _a === void 0 ? void 0 : _a();
                untrack === null || untrack === void 0 ? void 0 : untrack.delete(server);
                onRemove(server, k);
            });
            return () => {
                addOff === null || addOff === void 0 ? void 0 : addOff();
                ...
                if (untrack !== undefined) {
                    for (const off of untrack.values())
                        off();
                    untrack.clear();
                }
            };
     ...
     */
    bindInputRenderDelay(input) {
        if (input instanceof InputHandle.InputHandleImpl) {
    ...
    reconciler(instance, opts) {
        ...
        const recon = new reconciler.Reconciler(instance, Object.assign(Object.assign({}, opts), { clock: (_a = opts.clock) !== null && _a !== void 0 ? _a : this.clock }));
    ...
    sim(opts) {
        ...
        const ctl = new simReconciler.SimReconciler(Object.assign(Object.assign({}, opts), { clock: (_a = opts.clock) !== null && _a !== void 0 ? _a : this.clock }));
        ...
        this.installBoundOverlay(ctl);
        this.driven.push(ctl);
        return ctl;
    }
    /**
    ...
    adoptFixedStep(stepMs) {
        ...
        if (Math.abs(this.fixedStepMs - stepMs) > 1e-6) {
            console.warn(`@colyseus/sdk Predict: a reconciler's fixed step (${stepMs}ms) differs from ` +
    ...
    value(instance, field) {
        ...
        const refId = schemaReflect.refIdOf(instance);
        const slotIdx = refId === undefined ? undefined : (_a = this.slotByRef.get(refId)) === null || _a === void 0 ? void 0 : _a.get(field);
    ...
    valueAt(instance, field, time) {
        ...
        if (sim !== undefined) {
            ...
            if (pos >= 0) {
                ...
                const base = (_d = (_c = (_b = this.clock) === null || _b === void 0 ? void 0 : _b.lastServerTime) === null || _c === void 0 ? void 0 : _c.call(_b)) !== null && _d !== void 0 ? _d : NaN;
     ...
     */
    read(instance, fields, out) {
        const o = (out !== null && out !== void 0 ? out : {});
        const refId = schemaReflect.refIdOf(instance);
        const fieldMap = refId === undefined ? undefined : this.slotByRef.get(refId);
        for (let i = 0; i < fields.length; i++) {
            const field = fields[i];
            const slotIdx = fieldMap === null || fieldMap === void 0 ? void 0 : fieldMap.get(field);
            o[field] = slotIdx === undefined
                ? instance[field] // untracked → live, as value()
     ...
     */
    readAt(instance, fields, time, out) {
        var _a, _b, _c;
        const o = (out !== null && out !== void 0 ? out : {});
        const refId = schemaReflect.refIdOf(instance);
        const fieldMap = refId === undefined ? undefined : this.slotByRef.get(refId);
        ...
        for (let i = 0; i < fields.length; i++) {
            ...
            if (!advanced) {
                ...
                if (sim !== undefined) {
                    ...
                    const base = (_c = (_b = (_a = this.clock) === null || _a === void 0 ? void 0 : _a.lastServerTime) === null || _b === void 0 ? void 0 : _b.call(_a)) !== null && _c !== void 0 ? _c : NaN;
                    const fwd = Number.isNaN(base) ? 0 : Math.max(0, time - base);
                    sim.advance(sim.instance, fwd, sim.valueOut, time);
                }
            }
    ...
    computeLerpRaw(slotIdx) {
        ...
        if (target <= buf[oldestOff])
            return buf[oldestOff + 1];
        if (target >= buf[newestOff])
            return buf[newestOff + 1];
    ...
    applySimulation(sim, pos) {
        ...
        if (sim.lastApplyTime !== now) {
            ...
            const n = sm.length;
            const baseT = (_c = (_b = (_a = this.clock) === null || _a === void 0 ? void 0 : _a.lastServerTime) === null || _b === void 0 ? void 0 : _b.call(_a)) !== null && _c !== void 0 ? _c : NaN;
