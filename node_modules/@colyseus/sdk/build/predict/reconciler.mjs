...
import { scalarFieldsOf, wireQuantizerOf } from '../core/schema-reflect.mjs';
import { diagnosticsActive } from './divergence.mjs';
...
const asScalar = (v) => typeof v === "number" ? v : typeof v === "boolean" ? (v ? 1 : 0) : NaN;
class Reconciler extends RollbackController {
    ...
    constructor(instance, opts) {
        ...
        if (explicit === undefined && fields.length === 0) {
            throw new Error("predict.reconciler(): no `fields` given and none derivable — the " +
    ...
    get boundRegistrations() {
        return [{ source: this.instance, fields: this.numericFields, poseKeys: this.numericFields }];
    }
    /**
    ...
    value(field) {
        this.noteRenderRead();
        const c = this.local[field];
        if (typeof c !== "number")
            return c;
        const smoothed = c + (this.error[field] ?? 0);
        const p = this.prev[field] ?? smoothed;
        return p + (smoothed - p) * this.renderAlpha();
    }
    // --- RollbackController hooks ----------------------------------------------
    smoothedFields() { return this.numericFields; }
    readCurrent(field) { return this.local[field]; }
    applyStep(input) {
     ...
     *  PORTING.md. */
    stepView() {
        if (this.devView !== null)
            return this.devView;
        const undeclared = this.undeclared;
        const warned = new Set();
        const check = (verb, prop) => {
            if (typeof prop !== "string" || !undeclared.has(prop) || warned.has(prop))
                return;
            warned.add(prop);
            console.warn(`@colyseus/sdk predict: step ${verb} "${prop}" — a schema field NOT in this ` +
     ...
     */
    truthMatchesAt(acked) {
        if (!this.historyOn)
            return false;
        const slot = acked % this.historySize;
        if (this.historySeq[slot] !== acked)
            return false;
        const base = slot * this.fields.length;
        const inst = this.instance;
        for (let i = 0; i < this.fields.length; i++) {
            if (this.wireRound[i](this.history[base + i]) !== asScalar(inst[this.fields[i]]))
                return false;
        }
        return true;
    }
    snapshotPrev() {
        for (const f of this.numericFields)
            this.prev[f] = this.local[f] + this.error[f];
    }
    adoptTruth() {
        const inst = this.instance;
        for (const f of this.fields)
