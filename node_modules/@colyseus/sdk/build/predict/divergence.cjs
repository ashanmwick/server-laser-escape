...
function diagnosticsActive() {
    return debugChannel.debugOverlayActive();
}
 ...
 */
function warnReadBeforePump() {
    console.warn(`@colyseus/sdk predict: a predicted value was read BEFORE this frame's input.send() calls. ` +
 ...
 */
function warnMemoCollision(tick, key) {
    console.warn(key === ""
 ...
 */
function warnDivergence(acked, lastCorrection, ema, tolerance) {
    const t = utils.now();
    if (t - _lastWarnAt < WARN_THROTTLE_MS) {
        return;
    }
    _lastWarnAt = t;
    let worst = "";
    ...
    console.warn(`@colyseus/sdk predict: prediction is diverging at input seq ${acked} — ` +
...
exports.diagnosticsActive = diagnosticsActive;
exports.warnDivergence = warnDivergence;
exports.warnMemoCollision = warnMemoCollision;
exports.warnReadBeforePump = warnReadBeforePump;
//# sourceMappingURL=divergence.cjs.map
