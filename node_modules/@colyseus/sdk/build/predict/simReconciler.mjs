...
//
// colyseus.js@0.18.2
import { RollbackController } from './rollback.mjs';
import { refIdOf, metadataOf, scalarFieldsOf } from '../core/schema-reflect.mjs';
 ...
 *     });
 */
class SimReconciler extends RollbackController {
    ...
    constructor(opts) {
        ...
        for (const part of Object.keys(world)) {
            ...
            if (refIdOf(src) === undefined) {
                if (metadataOf(src) !== undefined) {
                    throw new Error(`predict.sim(): world.${part} is a schema instance that hasn't been ` +
            ...
            if (fields.length === 0) {
                throw new Error(`predict.sim(): world.${part} is a decoded ref with no scalar fields ` +
        ...
        if (this.bindings.length === 0 && opts.adopt === undefined) {
            throw new Error("predict.sim(): no restore point — world has no schema-bound entries " +
    ...
    refreshPose() {
        ...
        for (const b of this.bindings) {
            ...
            for (let i = 0; i < numeric.length; i++)
                cur[poseKeys[i]] = mirror[numeric[i]];
        }
        if (this.readPose !== undefined) {
            const pose = this.readPose(this.worldHandle);
            if (!this.fieldsReady)
                this.customFields = Object.keys(pose);
    ...
    get world() { return this.worldHandle; }
    ...
    pose() {
        ...
        for (const f of this.poseFields) {
            a[f] = this.prev[f] ?? this.curPose[f];
            b[f] = this.curPose[f] + (this.error[f] ?? 0);
        }
        if (this.interpolate) {
            const out = this.interpolate(a, b, t);
            for (const f of this.poseFields)
                this.renderPose[f] = out[f];
        }
        else {
            for (const f of this.poseFields)
                this.renderPose[f] = a[f] + (b[f] - a[f]) * t;
        }
        this.poseDirty = false;
        return this.renderPose;
    }
    // --- RollbackController hooks ----------------------------------------------
    smoothedFields() { return this.poseFields; }
    readCurrent(field) { return this.curPose[field]; }
    applyStep(input) { this.step(this.stepCtx, this.worldHandle, input); }
    ...
    adoptTruth() {
        for (const b of this.bindings) {
            ...
            for (let i = 0; i < fields.length; i++)
                mirror[fields[i]] = source[fields[i]];
        }
        this.adopt?.(this.worldHandle);
    }
    refreshRender() { this.refreshPose(); }
    markDirty() { this.poseDirty = true; }
    snapshotPrev() {
        for (const f of this.poseFields)
            this.prev[f] = this.curPose[f] + this.error[f];
    }
    reseedState() {
        this.adoptTruth();
        this.refreshPose();
...
export { SimReconciler };
//# sourceMappingURL=simReconciler.mjs.map
