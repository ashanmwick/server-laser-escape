...
// -----------------------------------------------------------------------------
var _a;
const $METADATA = (_a = Symbol.metadata) !== null && _a !== void 0 ? _a : Symbol.for("Symbol.metadata");
 ...
 *  file header). */
function refIdOf(instance) {
    return instance[schema.$refId];
}
...
function fieldIndexOf(instance, field) {
    var _a;
    const idx = (_a = metadataOf(instance)) === null || _a === void 0 ? void 0 : _a[field];
...
function scalarFieldNamesOf(instance) {
    ...
    for (let i = 0;; i++) {
        ...
        if (typeof f.type === "string")
            names.push(f.name); // skip collections / child schemas (object type)
    }
    return names;
}
 ...
 * non-schema objects.
 */
function scalarFieldsOf(instance) {
    const meta = metadataOf(instance);
    const fields = [];
    const numeric = [];
    if (meta) {
        for (let i = 0;; i++) {
            ...
            if (!f || typeof f.name !== "string")
                break;
            if (typeof f.type !== "string")
                continue; // skip collections / child schemas
            fields.push(f.name);
            if (NON_NUMERIC_SCALAR[f.type] === undefined)
                numeric.push(f.name);
        }
    }
 ...
 */
function quantizeAutoNumber(v) {
    if (Number.isNaN(v))
        return 0;
    if (!Number.isFinite(v))
        return v > 0 ? Number.MAX_SAFE_INTEGER : -Number.MAX_SAFE_INTEGER;
    if (v !== (v | 0)) {
        if (Math.abs(v) <= 3.4028235e+38) {
            const f = Math.fround(v);
            if (Math.abs(Math.abs(f) - Math.abs(v)) < 1e-4)
                return f;
        }
    }
    return v;
 ...
 */
function wireQuantizerOf(instance, field) {
    const meta = metadataOf(instance);
    const idx = meta === null || meta === void 0 ? void 0 : meta[field];
    const decl = typeof idx === "number" ? meta[idx] : undefined;
    switch (decl === null || decl === void 0 ? void 0 : decl.type) {
...
function makeUnrolledSnapshot(fieldNames) {
    ...
    const n0 = fieldNames[0], n1 = fieldNames[1], n2 = fieldNames[2], n3 = fieldNames[3], n4 = fieldNames[4], n5 = fieldNames[5], n6 = fieldNames[6], n7 = fieldNames[7], n8 = fieldNames[8], n9 = fieldNames[9], n10 = fieldNames[10], n11 = fieldNames[11], n12 = fieldNames[12], n13 = fieldNames[13], n14 = fieldNames[14], n15 = fieldNames[15];
    return (e) => {
        const o = {};
        if (n0 !== undefined)
            o[n0] = e[n0];
...
exports.makeUnrolledSnapshot = makeUnrolledSnapshot;
exports.metadataOf = metadataOf;
exports.refIdOf = refIdOf;
exports.scalarFieldNamesOf = scalarFieldNamesOf;
exports.scalarFieldsOf = scalarFieldsOf;
exports.wireQuantizerOf = wireQuantizerOf;
//# sourceMappingURL=schema-reflect.cjs.map
