...
var RedisDriver = class {
  ...
  async findByIds(roomIds) {
    ...
    for (let i = 0; i < roomIds.length; i++) {
      const raw = values[i];
      if (raw) {
        result.set(roomIds[i], (0, import_core.initializeRoomCache)(JSON.parse(raw)));
      }
    }
    return result;
  }
