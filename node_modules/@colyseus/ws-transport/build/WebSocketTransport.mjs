...
import { matchMaker, Protocol, Transport, createAuthContext, debugAndPrintError, debugConnection, CloseCode, connectClientToRoom, runBeforeUpgrade, isDevMode } from "@colyseus/core";
...
var WebSocketTransport = class extends Transport {
   ...
   */
  listenForUpgrade(server, filter) {
    const completeUpgrade = (req, socket, head, upgrade) => this.wss.handleUpgrade(req, socket, head, (ws) => this.wss.emit("connection", ws, req, upgrade));
    server.on("upgrade", (req, socket, head) => {
      ...
      const upgrade = { url, context: buildAuthContext(req, url) };
      ...
      runBeforeUpgrade(this._beforeUpgrade, req.url ?? "/", upgrade.context).then((response) => {
        ...
        if (response !== void 0) {
          return writeResponse(socket, response);
        }
  ...
  async onConnection(rawClient, req, upgrade) {
